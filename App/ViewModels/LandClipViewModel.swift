...
final class LandClipViewModel: ObservableObject {
    ...
    var canDraw: Bool { stage == .ready || stage == .done }
    var canClip: Bool {
        switch stage {
        case .ready, .done: return aoiVertices.count >= 3
    ...
    func runClip() {
        guard case .ready = stage, let prepared, aoiVertices.count >= 3 else { return }
        progress = Report()
        result = nil
        stage = .clipping
        ...
        worker = Task {
            do {
                ...
                let aoiURL = outputDirectory.appendingPathComponent("aoi.geojson")
